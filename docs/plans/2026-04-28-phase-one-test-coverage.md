...
**Implementation status as of 2026-05-02:** PR #5, PR #7, and the follow-up release-confidence PRs are merged into `development`. The Flutter suite now passes 139 tests across thirteen focused files with `flutter test --coverage`, and Maestro Android/iOS launch, Settings About, and Settings Coming Soon smoke flows exist under `luna_lighthouse/.maestro/`. Test-infrastructure enforcement now includes a dedicated CI `mobile-test` gate, coverage artifact upload, a 2% LCOV floor, and `flutter test` inside `scripts/mobile-build-check`.
