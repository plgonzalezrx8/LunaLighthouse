...
LunaLighthouse includes an on-device logging system that helps debug crashes, issues, and support requests. LunaLighthouse stores 4 different levels of logs: **Debug**, **Warning**, **Error**, and **Critical**.
...
{% tab title="Critical" %}
Critical logs are crashing errors or unhandled errors that have occurred on the device. Critical logs would occur when LunaLighthouse fails to complete the boot process, fails to render the UI, etc.
{% endtab %}
{% endtabs %}
...
LunaLighthouse also stores a list of the **last 100 logs** that have occurred on the device. To access the history of logs, go to Settings -> System -> Logs.

{% hint style="info" %}
The log database size is checked on startup, so all logs that have occurred in the active session will remain in the log history until LunaLighthouse is closed and reopened.
{% endhint %}
...
LunaLighthouse offers the ability to clear all recorded logs from your device. While an available option, it is not recommended to clear your logs often as LunaLighthouse will internally ensure the log database does not grow to an obscenely large size.
