...
extension DateTimeExtension on DateTime {
  ...
  String asDateTime({
    ...
    format.write(
        LunaLighthouseDatabase.USE_24_HOUR_TIME.read() ? 'HH:mm' : 'hh:mm');
  ...
  String asAge() {
    ...
    if (days >= 1) {
      ...
      if (years > 1)
        return 'luna_lighthouse.YearsAgo'.tr(args: [years.toString()]);

      final months = (days / 30).floor();
      if (months == 1) return 'luna_lighthouse.OneMonthAgo'.tr();
      if (months > 1)
        return 'luna_lighthouse.MonthsAgo'.tr(args: [months.toString()]);

      if (days == 1) return 'luna_lighthouse.OneDayAgo'.tr();
      if (days > 1)
        return 'luna_lighthouse.DaysAgo'.tr(args: [days.toString()]);
    }

    final hours = diff.inHours.abs();
    if (hours == 1) return 'luna_lighthouse.OneHourAgo'.tr();
    if (hours > 1)
      return 'luna_lighthouse.HoursAgo'.tr(args: [hours.toString()]);

    final mins = diff.inMinutes.abs();
    if (mins == 1) return 'luna_lighthouse.OneMinuteAgo'.tr();
    if (mins > 1)
      return 'luna_lighthouse.MinutesAgo'.tr(args: [mins.toString()]);
  ...
  String asDaysDifference() {
    ...
    if (months > 1)
      return 'luna_lighthouse.Months'.tr(args: [months.toString()]);
