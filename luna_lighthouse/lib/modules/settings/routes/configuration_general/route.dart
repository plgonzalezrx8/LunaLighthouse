...
class _State extends State<ConfigurationGeneralRoute>
  ...
  Widget _openDrawerOnBackAction() {
    const _db = LunaLighthouseDatabase.ANDROID_BACK_OPENS_DRAWER;
  ...
  Widget _amoledTheme() {
    const _db = LunaLighthouseDatabase.THEME_AMOLED;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
  ...
  Widget _amoledThemeBorders() {
    return LunaBox.luna_lighthouse.listenableBuilder(
      selectItems: [
      ...
      builder: (context, _) => LunaBlock(
        ...
        trailing: LunaSwitch(
          value: LunaLighthouseDatabase.THEME_AMOLED_BORDER.read(),
          onChanged: LunaLighthouseDatabase.THEME_AMOLED.read()
              ? LunaLighthouseDatabase.THEME_AMOLED_BORDER.update
              : null,
        ),
      ),
    );
  ...
  Widget _imageBackgroundOpacity() {
    const _db = LunaLighthouseDatabase.THEME_IMAGE_BACKGROUND_OPACITY;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
  ...
  Widget _useTLSValidation() {
    const _db = LunaLighthouseDatabase.NETWORKING_TLS_VALIDATION;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
  ...
  Widget _use24HourTime() {
    const _db = LunaLighthouseDatabase.USE_24_HOUR_TIME;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
