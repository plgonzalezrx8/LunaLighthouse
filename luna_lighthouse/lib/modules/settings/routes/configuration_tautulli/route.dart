  ...
  Widget _activityRefreshRate() {
    ...
    return _db.listenableBuilder(builder: (context, _) {
      String refreshRate = _db.read() == 1
          ? 'luna_lighthouse.EverySecond'.tr()
          : 'luna_lighthouse.EverySeconds'.tr(args: [_db.read().toString()]);
      return LunaBlock(
    ...
    return _db.listenableBuilder(
      builder: (context, _) {
        String statisticsItems = _db.read() == 1
            ? 'luna_lighthouse.OneItem'.tr()
            : 'luna_lighthouse.Items'.tr(args: [_db.read().toString()]);
        return LunaBlock(
