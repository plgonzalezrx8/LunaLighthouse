...
void main() {
  ...
  group('LunaModule route metadata', () {
    ...
    test('homeRoute values are registered root paths or null', () {
      final registeredPaths = LunaRoutes.values
          .map((route) => route.root.path)
          .toSet();
      ...
      for (final module in LunaModule.values) {
        ...
        if (home != null) {
          expect(
            registeredPaths,
            contains(home),
            reason: '${module.key}.homeRoute ($home) not found in route registry',
          );
        }
      }
    });
  ...
  group('LunaRoutes count', () {
    test('total route count matches expected phase-one registry size', () {
      // Changing this number without updating tests signals that the route
      // registry diverged from the documented architecture.
      expect(LunaRoutes.values.length, equals(11));
    });
  });
}
