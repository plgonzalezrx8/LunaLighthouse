...
Alongside cloud-based backups, Lunarr offers the ability to create local backups of your configuration which you can manually transfer between devices and restore from. This gives you the freedom to have complete control of your data, and never have your configuration saved in a cloud server. Local backups are fully encrypted on-device, there is currently no option to create a non-encrypted backup.
...
Creating a backup in Lunarr is simple and painless! Simply go to Settings -> System -> "Backup to Device" to start the backup process.
...
Restoring a backup in Lunarr is as simple as creating the backup! Simply go to Settings -> System -> "Restore from Device" to start the restore process.
